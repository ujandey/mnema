...
\begin{titlepage}
\thispagestyle{empty}
{\large\scshape COGNX Research\par}
\vspace{8mm}
\hrule height 0.5pt
\vspace{26mm}
{\large COGNX Technical Report 001\par}
\vspace{12mm}
...
% Page 2: abstract, question, and evidence boundary.
\begin{abstract}
...
\reportpage
% Page 3: architecture and input/fast-memory mechanisms.
...
\reportpage
% Page 5: protocol, algorithms, and metrics.
\section{Experimental Protocol}
\label{sec:protocol}
\subsection{Split-MNIST class-incremental learning}
The completed full benchmark uses the fixed task order
$01\rightarrow23\rightarrow45\rightarrow67\rightarrow89$, all 60,000
training images, and all 10,000 test images. Seeds 0--9 give ten replications
for each of six methods. Within each seed, methods receive identical shuffled
training indices, with one external presentation of each training example.
All five test tasks are evaluated after each training task, including tasks not
yet trained. Every prediction considers \textbf{all ten classes}; no inference
task ID, output masking, or task-specific head is used~\citep{mnemaBenchmark}.

\subsection{Compared methods}
The five dense baselines share a $784\rightarrow256$ ReLU $\rightarrow10$
\code{float32} MLP, identical initial parameters per seed, and SGD with learning
rate 0.01. There is no momentum or pretrained backbone. All reported settings
are fixed defaults, without validation-based hyperparameter search or
final-test-set tuning.

\textbf{Naive MLP} makes a cross-entropy (CE) update on each incoming example.
\textbf{Replay-300} (named ``Replay-300 (Research)'' in the artifacts) maintains
a true reservoir of 300 \code{uint8} images and labels. It samples one previous
example before insertion and makes one update on mean CE over the current and
replayed examples, when the reservoir is nonempty. \textbf{Replay-64KiB} uses
the same policy with a strict 65,536-byte allocated buffer-payload budget,
including image, label, and size/seen counters. Its derived capacity is 83
images. This is not exact total-memory matching with MNEMA.

\textbf{EWC} adds a task-wise diagonal Fisher penalty with $\lambda=100$,
retaining separate past-task parameter snapshots and Fishers~\citep{kirkpatrick2017}.
Here the empirical diagonal Fisher averages squared per-example CE gradients
with observed labels. All training examples of each of the first four tasks
are reread for estimation, without optimizer updates; no Fisher is built after
the last task. EWC receives training task boundaries and is not a strictly
single-access stream algorithm.

\textbf{DER++-300} keeps a 300-item reservoir of images, labels, and ten
\code{float32} logits captured before the original sample's optimizer update.
Its loss is current CE plus $0.5$ times mean squared logit error plus $0.5$
times replay-label CE. The two replay terms draw independent prior examples
and contribute to one combined update~\citep{buzzega2020}.
\textbf{MNEMA} uses the defaults in Section~\ref{sec:architecture}, including
internal consolidation. One external pass does not mean equal compute:
rehearsal, Fisher estimation, and consolidation add different amounts of work.

\subsection{Checkpoint-isolated MNEMA evaluation}
Native \code{is_training=False} skips learning and eligibility maintenance,
but still changes encoder threshold, separator homeostasis, cortex membrane
and adaptation, and controller state. Native predictions can depend on prior
inputs. The research adapter snapshots the inference-mutable state and RNG
state and restores them in \code{finally} after every test image, including on
exceptions. The trained checkpoint's existing membrane state is preserved;
it is not reset to zero. Predictions receive neither labels nor task IDs.

Full attribute-state hashes are compared before and after each evaluation set.
At the final MNEMA checkpoint, reverse-order predictions are checked image by
image. This isolation is \textbf{part of the evaluation protocol}, not a
property conferred by MNEMA's native inference flag.
...
\source{Full benchmark report and configuration; \code{research_state.py},
\code{research_results.py}, \code{run_research_benchmark.py} under
\code{experiments/}; research baselines under \code{baselines/}.}

\reportpage
% Page 6: original results and retention plot.
\section{Results}
\label{sec:results}
...
\reportpage
% Page 7: accounting with all categories from the original table.
...
\reportpage
% Page 8: balanced discussion and original auxiliary-memory plot.
\section{Discussion}
...
\reportpage
% Page 9: limitations, proposed next experiments, modest conclusion.
\section{Limitations}
...
\reportpage
% Page 10: reproducibility and bibliography.
\appendix
\section{Reproducibility and Evidence Map}
\label{sec:repro}
Repository: \url{https://github.com/ujandey/mnema}.
The canonical full command, verified against the runbook and runner, is:
\begin{quote}\small\ttfamily
python experiments/run\_research\_benchmark.py --mode full --seeds 10
\end{quote}
Run from the repository root in a compatible research environment. This
command executes all 60 pairs sequentially; the saved bundle records
independent CPU worker jobs on GitHub Actions. The original configuration was
created on September 11, 2026, at 19:38:47 UTC. It records Python 3.11.7,
NumPy 2.2.6, Matplotlib 3.10.3, PyYAML 6.0.2, and one BLAS thread. The
standalone pins are in \code{experiments/research_requirements.txt}; the
Python 3.14 package environment is a separate definition.

\textbf{Recorded benchmark source commit:}\par
\nolinkurl{fe86ec1c6abc600dda8ec50565a551af4e5434bd}\par
\textbf{Checkout audited for this report:}\par
\nolinkurl{0d317c0b75f82fe9a599a84a469684e991259806}

Exact configuration, source SHA-256 hashes, dataset checksums, raw results,
per-seed indices, summaries, validation, and worker records are preserved under
\code{results/research_benchmark/full/}. The import audit reports saved-artifact
checks passing, with both training and inference reruns false. It recorded a
runner content difference and line-ending differences; later license-header
updates introduce further source-hash differences. Standard source validation
and resume reject this bundle on the current checkout. Exact reproduction
requires the recorded source bytes and environment. A new current-source run
requires a separate checkout/output context that preserves the saved bundle;
recorded hashes must not be edited to bypass checks.

The empirical authority is \code{BENCHMARK_REPORT.md}; \code{config.json},
\code{summaries/}, and \code{plots/} provide supporting configuration and
artifacts. Architecture is traced to \code{docs/ARCHITECTURE.md} and
\code{mnema/}; evaluation and accounting to
\code{experiments/research_state.py}, \code{research_results.py},
\code{research_memory.py}, and \code{instrument/}. Setup and execution follow
\code{docs/GETTING_STARTED.md}, \code{docs/RESEARCH_BENCHMARK.md}, and
\code{experiments/RESEARCH_BENCHMARK.md}. The accompanying report README
records the complete audit map, figure checksums, omissions, and build status.
No research implementation or benchmark data was modified for this report.
