...
**Current-source limitation:** the current `experiments/run_research_benchmark.py` does not match its recorded source hash even after line-ending normalization. Eighteen additional source files differ only in CRLF/LF encoding. Consequently the unmodified standard validator and resume logic reject this bundle on this checkout. This does not change the successful saved-artifact consistency checks, but it prevents a claim of exact reproduction with the current source.
